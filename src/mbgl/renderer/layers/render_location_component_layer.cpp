...
struct LocationComponentRenderParameters {
    LocationComponentRenderParameters() = default;
    explicit LocationComponentRenderParameters(const TransformParameters& tp) : state(&tp.state) {}
    LocationComponentRenderParameters(const LocationComponentRenderParameters& o) = default;
    LocationComponentRenderParameters& operator=(const LocationComponentRenderParameters& o) = default;
...
protected:
    struct vec2 {
        ...
        vec2(const vec2&& o) noexcept : x(o.x), y(o.y) {}
        ...
        Point<double> point() const { return {x, y}; }

        friend vec2 operator-(const vec2& v) { return vec2(-v.x, -v.y); }
        friend vec2 operator*(double a, const vec2& v) { return vec2(v.x * a, v.y * a); }
...
public:
        ...
        */
        ...
        void upload() {
            ...
            if (!image || !image->valid()) {
                ...
                if (RenderLocationComponentImpl::anisotropicFilteringAvailable)
    ...
    RenderLocationComponentImpl() : ruler(0, mapbox::cheap_ruler::CheapRuler::Meters) {}
    ...
    void render(const mbgl::LocationComponentRenderParameters& params) {
        initialize();
        drawRadius(params);
        drawShadow();
        drawPuck();
    ...
    void updatePuckGeometry(const mbgl::LocationComponentRenderParameters& params) {
        ...
        if (params.puckBearing != oldParams.puckBearing) bearingChanged = true;
...
protected:
    ...
    static void toMercator(mat4& mbgl_proj, const double zoom, const double latitude) {
        // Scale by worldSize so that the values in the input coodinate system are between 0 and 1.
        const double worldSize = std::pow(2, zoom) * 512;
        for (size_t i = 0; i < 12; i++) {
            mbgl_proj.at(i) *= worldSize;
        }

        // Undo the z scaling done in GL native that makes z coordinates match altitude in meters. This makes the
        // projection conformant, i.e., a cube in the input coordinate system is a cube in the output.
        double zScale = mbgl::Projection::getMetersPerPixelAtLatitude(latitude, zoom);
        for (size_t i = 8; i < 12; i++) {
            mbgl_proj.at(i) *= zScale;
        }
    }

    void updateRadius(const mbgl::LocationComponentRenderParameters& params) {
    ...
    static Point<double> hatShadowShiftVector(const LatLng& position,
                                              const mbgl::LocationComponentRenderParameters& params) {
        const TransformState& s = *params.state;
        ...
        return Point<double>(verticalShiftAtPos.x, verticalShiftAtPos.y);
    }
    ...
    static vec2 directionAtPositionScreen(const LatLng& position, float bearing, const TransformState& s) {
        ...
        Point<double> posMercDelta = posMerc + rot.point();
        ScreenCoordinate posScreen = latLngToScreenCoordinate(position, s);
    ...
    void updatePuck(const mbgl::LocationComponentRenderParameters& params) { return updatePuckPerspective(params); }

    void updatePuckPerspective(const mbgl::LocationComponentRenderParameters& params) {
        const TransformState& s = *params.state;
        ...
        for (unsigned long i = 0; i < 4; ++i) {
            const float b = util::wrap<float>(params.puckBearing + bearings[i], 0.0f, 360.0f);
    ...
    void drawRadius(const mbgl::LocationComponentRenderParameters& params) {
        ...
        if (params.errorRadiusBorderColor.a > 0.0f) {
            ...
            MBGL_CHECK_ERROR(glDrawArrays(GL_LINE_STRIP, 1, circle.size() - 1)); // line width: 1 by default;
        }
    ...
    mbgl::LocationComponentRenderParameters oldParams;
    bool initialized = false;

public:
    mbgl::LocationComponentRenderParameters parameters;
    static bool anisotropicFilteringAvailable;
};

bool RenderLocationComponentImpl::anisotropicFilteringAvailable = false;

using namespace style;
namespace {

inline const LocationComponentLayer::Impl& impl(const Immutable<style::Layer::Impl>& impl) {
    assert(impl->getTypeInfo() == LocationComponentLayer::Impl::staticTypeInfo());
    return static_cast<const LocationComponentLayer::Impl&>(*impl);
}
} // namespace

RenderLocationComponentLayer::RenderLocationComponentLayer(Immutable<style::LocationComponentLayer::Impl> _impl)
    : RenderLayer(makeMutable<LocationComponentLayerProperties>(std::move(_impl))),
      renderImpl(new RenderLocationComponentImpl()),
...
RenderLocationComponentLayer::~RenderLocationComponentLayer() {
    assert(gfx::BackendScope::exists());
    if (!contextDestroyed) MBGL_CHECK_ERROR(renderImpl->deinitialize());
}

void RenderLocationComponentLayer::transition(const TransitionParameters& parameters) {
    unevaluated = impl(baseImpl).paint.transitioned(parameters, std::move(unevaluated));
}

void RenderLocationComponentLayer::evaluate(const PropertyEvaluationParameters& parameters) {
    passes = RenderPass::Translucent;
    auto properties = makeMutable<LocationComponentLayerProperties>(
        staticImmutableCast<LocationComponentLayer::Impl>(baseImpl), unevaluated.evaluate(parameters));
    ...
    if (!layout.get<style::AccuracyRadius>().isUndefined())
        renderImpl->parameters.errorRadiusMeters = layout.get<style::AccuracyRadius>().asConstant();
    if (!layout.get<style::BearingImageSize>().isUndefined())
        renderImpl->parameters.puckSizePx = layout.get<style::BearingImageSize>().asConstant();
    if (!layout.get<style::TopImageSize>().isUndefined())
        renderImpl->parameters.puckHatSizePx = layout.get<style::TopImageSize>().asConstant();
    if (!layout.get<style::ShadowImageSize>().isUndefined())
        renderImpl->parameters.puckShadowSizePx = layout.get<style::ShadowImageSize>().asConstant();
    ...
    if (!layout.get<style::Location>().isUndefined()) {
        const std::array<double, 3> pos = layout.get<style::Location>().asConstant();
...
bool RenderLocationComponentLayer::hasTransition() const {
    return unevaluated.hasTransition();
}
bool RenderLocationComponentLayer::hasCrossfade() const {
    return false;
}

void RenderLocationComponentLayer::markContextDestroyed() {
    contextDestroyed = true;
}

void RenderLocationComponentLayer::prepare(const LayerPrepareParameters& p) {
    ...
    renderImpl->parameters.fieldOfView = state.getFieldOfView();
    mat4 projMatrix;
...
void RenderLocationComponentLayer::render(PaintParameters& paintParameters) {
