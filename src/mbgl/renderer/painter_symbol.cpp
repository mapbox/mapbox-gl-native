                        ...
                        void (SymbolBucket::*drawSDF)(SDFShader&))
{
    ...
    const float angleOffset = aligned_with_map ? transform.currentState().getAngle() : 0;
    ...
    float angle = std::round(transform.currentState().getAngle() / M_PI * 128);
    ...
    sdfShader.u_zoom = (transform.currentState().getNormalizedZoom() - zoomAdjust) * 10; // current zoom level
    ...
    sdfShader.u_fadezoom = (transform.currentState().getNormalizedZoom() + f.bump) * 10;

    // The default gamma value has to be adjust for the current pixelratio so that we're not
    // drawing blurry font on retina screens.
    const float gamma = 0.105 * sdfFontSize / fontSize / transform.currentState().getPixelRatio();
    ...
    if (bucket.hasIconData()) {
        ...
        const float angleOffset =
            bucket.properties.icon.rotation_alignment == RotationAlignmentType::Map
                ? transform.currentState().getAngle()
                : 0;
        ...
        spriteAtlas.bind(transform.currentState().isChanging() || bucket.properties.placement == PlacementType::Line || angleOffset != 0 || fontScale != 1 || sdf);
        ...
        if (sdf) {
            ...
            // Convert the -pi..pi to an int8 range.
            const float angle = std::round(transform.currentState().getAngle() / M_PI * 128);
            ...
            iconShader->u_zoom = (transform.currentState().getNormalizedZoom() - zoomAdjust) * 10; // current zoom level

            iconShader->u_fadedist = 0 * 10;
            iconShader->u_minfadezoom = transform.currentState().getNormalizedZoom() * 10;
            iconShader->u_maxfadezoom = transform.currentState().getNormalizedZoom() * 10;
            iconShader->u_fadezoom = transform.currentState().getNormalizedZoom() * 10;
