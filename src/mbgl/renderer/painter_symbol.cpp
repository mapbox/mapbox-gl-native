                        ...
                        void (SymbolBucket::*drawSDF)(SDFShader&))
{
    ...
    const float angleOffset = aligned_with_map ? transform.finalState().getAngle() : 0;
    ...
    float angle = std::round(transform.finalState().getAngle() / M_PI * 128);
    ...
    sdfShader.u_zoom = (transform.finalState().getNormalizedZoom() - zoomAdjust) * 10; // current zoom level
    ...
    sdfShader.u_fadezoom = (transform.finalState().getNormalizedZoom() + f.bump) * 10;

    // The default gamma value has to be adjust for the current pixelratio so that we're not
    // drawing blurry font on retina screens.
    const float gamma = 0.105 * sdfFontSize / fontSize / transform.finalState().getPixelRatio();
...
void Painter::renderSymbol(SymbolBucket &bucket, util::ptr<StyleLayer> layer_desc, const Tile::ID &id, const mat4 &matrix) {
    ...
    if (bucket.hasIconData()) {
        ...
        spriteAtlas.bind(transform.finalState().isChanging() || bucket.properties.placement == PlacementType::Line || angleOffset != 0 || fontScale != 1 || sdf);
        ...
        if (sdf) {
            ...
            // Convert the -pi..pi to an int8 range.
            const float angle = std::round(transform.finalState().getAngle() / M_PI * 128);
            ...
            iconShader->u_zoom = (transform.finalState().getNormalizedZoom() - zoomAdjust) * 10; // current zoom level

            iconShader->u_fadedist = 0 * 10;
            iconShader->u_minfadezoom = transform.finalState().getNormalizedZoom() * 10;
            iconShader->u_maxfadezoom = transform.finalState().getNormalizedZoom() * 10;
            iconShader->u_fadezoom = transform.finalState().getNormalizedZoom() * 10;
