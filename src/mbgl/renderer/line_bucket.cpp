...
void LineBucket::addGeometry(const GeometryCoordinates& vertices) {
    ...
    if (closed) {
        ...
        nextNormal = util::perp(util::unit(Point<double>(firstVertex - *currentVertex)));
    }
    ...
    for (GLsizei i = 0; i < len; ++i) {
        ...
        nextNormal = nextVertex ? util::perp(util::unit(Point<double>(*nextVertex - *currentVertex)))
                                : prevNormal;
        ...
        if (isSharpCorner && i > 0) {
            ...
            if (prevSegmentLength > 2.0 * sharpCornerOffset) {
                GeometryCoordinate newPrevVertex = *currentVertex - GeometryCoordinate(util::round(Point<double>(*currentVertex - *prevVertex) * (sharpCornerOffset / prevSegmentLength)));
        ...
        if (isSharpCorner && i < len - 1) {
            ...
            if (nextSegmentLength > 2 * sharpCornerOffset) {
                GeometryCoordinate newCurrentVertex = *currentVertex + GeometryCoordinate(util::round(Point<double>(*nextVertex - *currentVertex) * (sharpCornerOffset / nextSegmentLength)));
