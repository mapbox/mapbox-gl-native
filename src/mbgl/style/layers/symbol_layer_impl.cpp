...
bool SymbolLayer::Impl::recalculate(const CalculationParameters& parameters) {
    ...
    // text-size and icon-size are layout properties but they also need to be evaluated as paint
    // properties:
    ...
    passes = ((paint.iconOpacity > 0 &&
               (paint.iconColor.value.a > 0 || paint.iconHaloColor.value.a > 0) && iconSize > 0) ||
              (paint.textOpacity > 0 &&
               (paint.textColor.value.a > 0 || paint.textHaloColor.value.a > 0) && textSize > 0))
...
std::unique_ptr<SymbolLayout> SymbolLayer::Impl::createLayout(BucketParameters& parameters) const {
    ...
    return std::make_unique<SymbolLayout>(
        id, parameters.layer.getName(), parameters.tileID.overscaleFactor(),
        parameters.tileID.overscaledZ, parameters.mode, parameters.layer, filter, layoutProperties,
        textMaxSize, *spriteAtlas);
}
