...
void Map::setPitch(double pitch, const AnimationOptions& animationOptions) {
    impl->cameraMutated = true;
    setPitch(pitch, {}, animationOptions);
}

void Map::setPitch(double pitch, optional<ScreenCoordinate> anchor, const AnimationOptions& animationOptions) {
    impl->cameraMutated = true;
    impl->transform.setPitch(pitch * util::DEG2RAD, anchor, animationOptions);
    impl->onUpdate(Update::Repaint);
}
