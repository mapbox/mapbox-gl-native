...
void LineBucket::drawLines(GLState &state, LineShader& shader) {
...
void LineBucket::drawPoints(GLState &state, LinejoinShader& shader) {
    char *vertex_index = BUFFER_OFFSET(vertex_start * vertexBuffer.itemSize);
    ...
    for (point_group_type& group : pointGroups) {
        ...
        group.array[0].bind(state, shader, vertexBuffer, pointElementsBuffer, vertex_index);
