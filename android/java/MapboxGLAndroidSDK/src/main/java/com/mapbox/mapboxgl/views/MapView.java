...
public class MapView extends FrameLayout implements LocationListener {
	...
	// Called when properties are being set from XML
	public MapView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle) {
	...
	public MapView(@NonNull Context context, @Nullable final AttributeSet attrs, int defStyleAttr, int defStyleRes) {
	...
	private void initialize(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		...
		// Load the attributes
		TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.MapView, defStyleAttr, defStyleRes);
		try {
	...
	public void setSprite(@NonNull String symbol, float scale, @NonNull Bitmap bitmap) {
	...
	public List<Polygon> addPolygons(@NonNull List<PolygonOptions> polygonOptions) {
	...
	public List<Annotation> getAnnotationsInBounds(@NonNull BoundingBox bbox) {
	...
	private void validateStyleUrl(@NonNull String url) {
	...
	public boolean onTouchEvent(@NonNull MotionEvent event) {
		...
		if ((android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) && (event.getButtonState() != 0) &&
	...
	private void rotateImageView(ImageView imageView, float angle) {
		...
		matrix.setScale((float) imageView.getWidth() / (float) imageView.getDrawable().getIntrinsicWidth(),
				(float) imageView.getHeight() / (float) imageView.getDrawable().getIntrinsicHeight());
