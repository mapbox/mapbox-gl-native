...
// Old version of GCC don't have __is_trivially_constructible, so we're using a minimal polyfill
// with GCC 4.x's based on __has_trivial_constructor intrinsic and a few known good arguments.
template <typename T, typename... Args>
struct is_trivially_constructible : std::false_type {};
template <typename T> struct is_trivially_constructible<T> : std::integral_constant<bool, __has_trivial_constructor(T)> {};
template <typename T> struct is_trivially_constructible<T, T&&> : std::integral_constant<bool, std::is_scalar<T>::value> {};
template <typename T> struct is_trivially_constructible<T, const T&> : std::integral_constant<bool, std::is_scalar<T>::value> {};
template <typename T> struct is_trivially_constructible<T, T&> : std::integral_constant<bool, std::is_scalar<T>::value> {};
#elif __has_feature(is_trivially_constructible)
// We're using the newer __is_trivially_constructible intrinsic
template <typename T, typename... Args>
struct is_trivially_constructible : std::integral_constant<bool, __is_trivially_constructible(T, Args...)> {};
#else
// Try our luck with the STL's version
template <typename T, typename... Args>
using is_trivially_constructible = std::is_trivially_constructible<T, Args...>;
#endif

// We're using a polyfill based on our is_trivially_constructible polyfill.
template <typename T> struct is_trivially_copy_constructible : is_trivially_constructible<T, typename std::add_lvalue_reference<typename std::add_const<T>::type>::type> {};
template <typename T> struct is_trivially_move_constructible : is_trivially_constructible<T, typename std::add_rvalue_reference<T>::type> {};
...
#include <vector>
namespace base {
template <typename... T> struct is_trivially_copy_constructible<std::vector<T...>> : std::false_type {};
#endif

} // namespace base

// replacement for #include "base/thread_annotations.h"
...
#undef OPTIONAL_DECLSPEC_EMPTY_BASES
  ...
  OPTIONAL_CONSTEXPR const T* operator->() const {
    DCHECK(storage_.is_populated_);
    return &storage_.value_;
  }

  OPTIONAL_CONSTEXPR T* operator->() {
    DCHECK(storage_.is_populated_);
    return &storage_.value_;
  }
  ...
  OPTIONAL_CONSTEXPR T& value() & {
    CHECK(storage_.is_populated_);
    return storage_.value_;
  }

  OPTIONAL_CONSTEXPR const T& value() const & {
    CHECK(storage_.is_populated_);
    return storage_.value_;
  }
