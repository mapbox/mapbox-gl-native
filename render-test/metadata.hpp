...
struct TestPaths {
    TestPaths() = default;
    TestPaths(mbgl::filesystem::path stylePath_, std::vector<mbgl::filesystem::path> expectations_, std::vector<mbgl::filesystem::path> expectedMetrics_)
        : stylePath(std::move(stylePath_)), expectations(std::move(expectations_)),  expectedMetrics(std::move(expectedMetrics_)) {}
