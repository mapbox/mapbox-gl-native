...
public:
    ...
    LatLng wrapped() const {
        return { latitude, longitude, Wrapped };
    }
...
public:
    ...
    ProjectedMeters(double n = 0, double e = 0) : northing(n), easting(e) {
    }
...
public:
    ...
    bool intersects(const LatLngBounds area) const {
        return (area.ne.latitude > sw.latitude && area.sw.latitude < ne.latitude &&
                area.ne.longitude > sw.longitude && area.sw.longitude < ne.longitude);
    }
