...
TEST(Memory, Vector) {
    ...
    Map map(test.view, test.fileSource, test.workerThreadPool, MapMode::Still);
...
TEST(Memory, Footprint) {
    ...
    for (unsigned i = 0; i < runs; ++i) {
        auto vector = std::make_unique<Map>(test.view, test.fileSource, test.workerThreadPool, MapMode::Still);
    ...
    for (unsigned i = 0; i < runs; ++i) {
        auto raster = std::make_unique<Map>(test.view, test.fileSource, test.workerThreadPool, MapMode::Still);
