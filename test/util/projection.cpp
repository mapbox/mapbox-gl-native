...
TEST(Projection, ProjectedMeters) {
    ...
    EXPECT_DOUBLE_EQ(projectedMeters.northing,
                     -Projection::projectedMetersForLatLng(southWest).northing);
    EXPECT_DOUBLE_EQ(projectedMeters.easting,
                     -Projection::projectedMetersForLatLng(southWest).easting);

    projectedMeters = ProjectedMeters{std::numeric_limits<double>::lowest(),
    ...
    projectedMeters =
        ProjectedMeters{std::numeric_limits<double>::max(), std::numeric_limits<double>::max()};
    latLng = Projection::latLngForProjectedMeters(projectedMeters);
    EXPECT_EQ(latLng.latitude, util::LATITUDE_MAX);
    EXPECT_EQ(latLng.longitude, util::LONGITUDE_MAX);
}
