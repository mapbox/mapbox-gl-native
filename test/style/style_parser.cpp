...
TEST_P(StyleParserTest, ParseStyle) {
    ...
    for (const auto& property : infoDoc.GetObject()) {
        ...
        if (value.HasMember("log")) {
            ...
            for (auto& js_entry : js_log.GetArray()) {
                ...
                const FixtureLogObserver::LogMessage message{
                    ...
                    *Enum<Event>::toEnum(js_entry[rapidjson::SizeType(2)].GetString()), int64_t(-1),
        ...
        if (unchecked.size()) {
            std::cerr << "Unchecked Log Messages (" << base << "/" << name << "): " << std::endl
                      << unchecked;
        }
                            ...
                            if (dir != nullptr) {
                                for (dirent* dp = nullptr; (dp = readdir(dir)) != nullptr;) {
                                    ...
                                    if (name.length() >= ending.length() &&
                                        name.compare(name.length() - ending.length(),
                                                     ending.length(), ending) == 0) {
                                        names.push_back(
                                            name.substr(0, name.length() - ending.length()));
                                    }
                                }
                                closedir(dir);
                            }
