...
TEST(Resource, Tile) {
    ...
    Resource vectorTile =
        Resource::tile("http://example.com/{z}/{x}/{y}.mvt", 2.0, 1, 2, 3, Tileset::Scheme::XYZ);
    ...
    Resource quadTile =
        Resource::tile("http://example.com/{quadkey}.png", 2.0, 0, 0, 1, Tileset::Scheme::XYZ);
    ...
    quadTile =
        Resource::tile("http://example.com/{quadkey}.png", 2.0, 0, 0, 2, Tileset::Scheme::XYZ);
    EXPECT_EQ(Resource::Kind::Tile, quadTile.kind);
    ...
    quadTile =
        Resource::tile("http://example.com/{quadkey}.png", 2.0, 1, 1, 2, Tileset::Scheme::XYZ);
    EXPECT_EQ(Resource::Kind::Tile, quadTile.kind);
    ...
    quadTile =
        Resource::tile("http://example.com/{quadkey}.png", 2.0, 29, 3, 6, Tileset::Scheme::XYZ);
    EXPECT_EQ(Resource::Kind::Tile, quadTile.kind);
    ...
    Resource tmsTile = Resource::tile("http://example.com/{z}/{x}/{y}{ratio}.png", 1.0, 1, 2, 3,
                                      Tileset::Scheme::TMS);
...
TEST(Resource, Glyphs) {
    using namespace mbgl;
    Resource resource =
        Resource::glyphs("http://example.com/{fontstack}/{range}", {{"stack"}}, {0, 255});
