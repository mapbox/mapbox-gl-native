...
def FlagsForFile(filename, **kwargs):
    ...
    working_directory = os.path.join('build', platform)

    if not os.path.exists(os.path.join(DirectoryOfThisScript(), working_directory)):
        return {'flags': default_flags, 'do_cache': False}
    ...
    make_cmd = 'make -pn |grep %s |grep NDEBUG |cut -d\' \' -f4- |head -n1' % relative_obj_path
    ...
    return {'flags': flags_final, 'do_cache': True}
