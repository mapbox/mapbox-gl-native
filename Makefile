...
app: deps config.gypi src macosx/llmr-app.gyp
	deps/run_gyp macosx/llmr-app.gyp -Icommon.gypi -Goutput_dir=./out/ --depth=. --generator-output=./build/macosx-make -f make
	make -C build/macosx-make V=$(V)
	open build/macosx-make/out/Release/llmr.app

linux: deps config.gypi src linux/llmr-app.gyp
	deps/run_gyp linux/llmr-app.gyp -Icommon.gypi -Goutput_dir=./out/ --depth=. --generator-output=./build/linux-make -f make
...
lproj: deps config.gypi src linux/llmr-app.gyp glfw
	deps/run_gyp linux/llmr-app.gyp -Icommon.gypi -Goutput_dir=./out/ --depth=. --generator-output=./ -f xcode
	open ./linux/llmr-app.xcodeproj

# build just xcode project for libllmr
xcode: deps config.gypi llmr.gyp glfw
	deps/run_gyp llmr.gyp -Icommon.gypi -Goutput_dir=./out/ --depth=. --generator-output=./ -f xcode

# build OS X app with Xcode
xproj: deps config.gypi src macosx/llmr-app.gyp glfw
	deps/run_gyp macosx/llmr-app.gyp -Icommon.gypi -Goutput_dir=./out/ --depth=. --generator-output=./ -f xcode
...
iproj: deps config.gypi src ios/llmr-app.gyp
	deps/run_gyp ios/llmr-app.gyp -Icommon.gypi -Goutput_dir=./out/ --depth=. --generator-output=./ -f xcode
