...
`MGLInterpolationModeIdentity` uses the attribute's value as the style value. For example, you can set the `circleRadius` to the earthquake's magnitude. Because the attribute value itself will be used as the style value, `sourceStops` can be set to `nil`.
...
Data-driven styling enables you to create powerful, custom maps that
