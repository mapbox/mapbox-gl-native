...
NodeLogObserver::NodeLogObserver(v8::Local<v8::Object> target)
    : queue(new Queue(uv_default_loop(), [this](LogMessage& message) {
          ...
          Nan::Set(msg, Nan::New("severity").ToLocalChecked(),
                   Nan::New(mbgl::Enum<mbgl::EventSeverity>::toString(message.severity))
                       .ToLocalChecked());
          ...
          auto emit = Nan::To<v8::Object>(
                          Nan::Get(handle, Nan::New("emit").ToLocalChecked()).ToLocalChecked())
                          .ToLocalChecked();
...
bool NodeLogObserver::onRecord(mbgl::EventSeverity severity,
                               mbgl::Event event,
                               int64_t code,
