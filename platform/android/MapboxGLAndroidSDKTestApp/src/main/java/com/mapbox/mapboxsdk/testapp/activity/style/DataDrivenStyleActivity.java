...
public class DataDrivenStyleActivity extends AppCompatActivity {
    ...
    layer.setProperties(
      fillColor(
        composite(
          "stroke-width",
          exponential(1f,
            stop(1, 1f, fillColor(Color.RED)),
            stop(1, 10f, fillColor(Color.BLUE)),
            stop(2, 2f, fillColor(Color.BLUE)),
            stop(2, 10f, fillColor(Color.RED))
          ))
      )
    );
  ...
  private void addCompositeCategoricalFunction() {
    ...
    FillLayer layer = mapboxMap.getLayerAs(AMSTERDAM_PARKS_LAYER);
    assert layer != null;
    Timber.i("It's: %s", fillColor(
      composite(
        "name",
        categorical(
          stop(14f, "Westerpark", fillColor(Color.RED)),
