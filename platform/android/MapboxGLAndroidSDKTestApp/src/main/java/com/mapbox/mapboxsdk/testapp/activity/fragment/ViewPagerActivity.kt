...
class ViewPagerActivity : AppCompatActivity() {
  ...
  override fun onRestoreInstanceState(savedInstanceState: Bundle) {
    ...
    if (Math.abs(0 - currentPosition) <= 1) {
      mapFragment = viewPager.adapter?.instantiateItem(viewPager, 0) as MapFragment
      mapFragment.getMapAsync { mapboxMap -> mapboxMap.setStyle(Style.MAPBOX_STREETS) }
    }

    if (Math.abs(1 - currentPosition) <= 1) {
      mapFragment = viewPager.adapter?.instantiateItem(viewPager, 1) as MapFragment
      mapFragment.getMapAsync { mapboxMap -> mapboxMap.setStyle(Style.DARK) }
    }

    if (Math.abs(2 - currentPosition) <= 1) {
      mapFragment = viewPager.adapter?.instantiateItem(viewPager, 2) as MapFragment
      mapFragment.getMapAsync { mapboxMap -> mapboxMap.setStyle(Style.SATELLITE) }
    }

    if (Math.abs(3 - currentPosition) <= 1) {
      mapFragment = viewPager.adapter?.instantiateItem(viewPager, 3) as MapFragment
      mapFragment.getMapAsync { mapboxMap -> mapboxMap.setStyle(Style.SATELLITE) }
    }

    if (Math.abs(4 - currentPosition) <= 1) {
      mapFragment = viewPager.adapter?.instantiateItem(viewPager, 4) as MapFragment
      mapFragment.getMapAsync { mapboxMap -> mapboxMap.setStyle(Style.SATELLITE) }
    }
  }
