...
public class MockParcel {
  ...
  private static class ParcelMocker {
    ...
    private void setupWrites() {
      ...
      Answer<Void> writeArrayAnswer = invocation -> {
      ...
      Answer<Void> writeIntArrayAnswer = invocation -> {
    ...
    private void setupReads() {
      when(mockedParcel.readInt()).then((Answer<Integer>) invocation -> (Integer) objects.get(position++));
      when(mockedParcel.readByte()).thenAnswer((Answer<Byte>) invocation -> (Byte) objects.get(position++));
      when(mockedParcel.readLong()).thenAnswer((Answer<Long>) invocation -> (Long) objects.get(position++));
      when(mockedParcel.readString()).thenAnswer((Answer<String>) invocation -> (String) objects.get(position++));
      when(mockedParcel.readDouble()).thenAnswer((Answer<Double>) invocation -> (Double) objects.get(position++));
      when(mockedParcel.readFloat()).thenAnswer((Answer<Float>) invocation -> (Float) objects.get(position++));
      when(mockedParcel.readParcelable(Parcelable.class.getClassLoader()))
        .thenAnswer((Answer<Parcelable>) invocation -> (Parcelable) objects.get(position++));
      when(mockedParcel.readParcelableArray(Parcelable.class.getClassLoader()))
      ...
      when(mockedParcel.createIntArray()).then((Answer<int[]>) invocation -> {
        int size = (Integer) objects.get(position++);
        if (size == -1) {
          return null;
        }
