...
/**
 ...
 height and width (measured in points) at which to display each tile in the
 source when the map’s zoom level is an integer.
 
 A tile must be square. The source may scale it up or down when the map’s zoom
 level falls between two integers.
 
 The default value for this option is 512 points on Retina displays and 256
 points otherwise. This option is only applicable to `MGLRasterSource` objects;
 it is ignored when initializing `MGLVectorSource` objects.
 */
